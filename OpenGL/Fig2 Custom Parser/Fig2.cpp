...
struct Point {
    Point(glm::vec3 setOutV, glm::vec2 setOutUV, glm::vec3 setOutN) {
        outV = setOutV;
        outUV = setOutUV;
        outN = setOutN;
...
bool ObjectLoader(const char *path) {
    ...
    while (1) {
        ...
        //READ MTL FILE
        if (strcmp(lineHeader, "spider.mtl") == 0) {
...
void loadTextures() {
    ...
    // Load the images.
    image[0] = getBMP("wal67ar_small.bmp");
    image[1] = getBMP("wal69ar_small.bmp");
    image[2] = getBMP("SpiderTex.bmp");
    image[3] = getBMP("drkwood2.bmp");
    image[4] = getBMP("engineflare1.bmp");
...
void drawScene() {
    ...
    for (auto &it:PartsMap) {
        for (int i = 0; i < it.second.size(); i += 3) { // Looping on each part
            ...
            if (currentMaterial.diffuseTextureFile == ".\\wal67ar_small.jpg") {
                //     cout<<"1"<<endl;
                glBindTexture(GL_TEXTURE_2D, texture[0]);
            } else if (currentMaterial.diffuseTextureFile == ".\\wal69ar_small.jpg") {
                //    cout<<"2"<<endl;
                glBindTexture(GL_TEXTURE_2D, texture[1]);
            } else if (currentMaterial.diffuseTextureFile == ".\\SpiderTex.jpg") {
                //      cout<<"3"<<endl;
                glBindTexture(GL_TEXTURE_2D, texture[2]);
            } else if (currentMaterial.diffuseTextureFile == ".\\drkwood2.jpg") {
                //      cout<<"4"<<endl;
                glBindTexture(GL_TEXTURE_2D, texture[3]);
            } else if (currentMaterial.diffuseTextureFile == ".\\engineflare1.jpg") {
                //      cout<<"5"<<endl;
                glBindTexture(GL_TEXTURE_2D, texture[4]);
            }
...
int main(int argc, char **argv) {
    bool res = ObjectLoader("./Spider.obj");
